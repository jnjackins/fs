...
func fsysLabel(fsys *Fsys, argv []string) error {
	...
	if argc == 6 {
		...
		if argv[3] != "-" {
			l.epoch = uint32(strtoul(argv[3], 0))
		}
		if argv[4] != "-" {
			l.epochClose = uint32(strtoul(argv[4], 0))
		}
		if argv[5] != "-" {
...
func cmdFsys(argv []string) error {
	...
	return fsysXXX(argv[0], []string(argv[1:]))
}
