...
func (f *Fcall) String() string {
	...
	case Rstat:
		...
		if err != nil {
			return fmt.Sprintf("Rstat tag=%d stat(%d bytes)", f.Tag, len(f.Stat))
		}
		return fmt.Sprintf("Rstat tag=%d stat=(%v)", f.Tag, d)
	case Twstat:
		d, err := UnmarshalDir(f.Stat)
		if err != nil {
			return fmt.Sprintf("Twstat tag=%d fid=%d stat(%d bytes)", f.Tag, f.Fid, len(f.Stat))
		}
